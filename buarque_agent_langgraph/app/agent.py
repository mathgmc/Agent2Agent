...
def generate_buarque_calendar() -> dict[str, list[str]]:
    """Generates Buarque's calendar for the next 7 days."""
    calendar = {}
    today = date.today()
    # Buarque's availability: evenings on weekdays, more free on weekends.
    for i in range(7):
...
BUARQUES_CALENDAR = generate_buarque_calendar()
...
def get_availability(date_range: str) -> str:
    """Use this to get Buarque's availability for a given date or date range."""
    ...
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = BUARQUES_CALENDAR.get(date_str, [])
            if available_slots:
                availability = (
                    f"On {date_str}, Buarque is available at: "
                    f"{', '.join(available_slots)}."
                )
                results.append(availability)
            else:
                results.append(f"Buarque is not available on {date_str}.")
...
class BuarqueAgent:
    """BuarqueAgent - a specialized assistant for scheduling."""

    SUPPORTED_CONTENT_TYPES = ["text", "text/plain"]

    SYSTEM_INSTRUCTION = (
        "You are Buarque's scheduling assistant. "
        "Your sole purpose is to use the 'get_availability' tool to answer questions about Buarque's schedule for a jam session. "
        ...
        "If the user asks about anything other than scheduling jam session, "
        ...
        for item in self.graph.stream(inputs, config, stream_mode="values"):
            ...
            if (
                ...
                yield {
                    ...
                    "content": "Checking Buarque's availability...",
                }
